...
use crate::proxy::stream::{all_channels_exhausted_response, json_response, sse_stream_response_with_telemetry, keepalive_stream};
use crate::router;
...
pub(crate) fn validate_chat_request(body: &Value) -> Result<(), Response> {
    if body.get("model").and_then(|m| m.as_str()).map_or(true, |s| s.is_empty()) {
    ...
    if body.get("messages").and_then(|m| m.as_array()).map_or(true, |a| a.is_empty()) {
        return Err(json_response(StatusCode::BAD_REQUEST, serde_json::json!({
...
pub(crate) fn extract_usage(body: &str) -> TokenUsage {
    let v: Value = match serde_json::from_str(body) {
        Ok(v) => v,
        Err(_) => return TokenUsage { input_tokens: None, output_tokens: None, cache_hit_tokens: None, cache_miss_tokens: None },
    };
    let usage = match v.get("usage") {
        Some(u) => u,
        None => return TokenUsage { input_tokens: None, output_tokens: None, cache_hit_tokens: None, cache_miss_tokens: None },
    };
    ...
    TokenUsage { input_tokens: input, output_tokens: output, cache_hit_tokens: cache_hit, cache_miss_tokens: cache_miss }
}
...
fn extract_usage_from_stream(chunks: &[String]) -> TokenUsage {
            ...
            if event_type == Some("message_start") {
                if let Some(message) = v.get("message") {
                    if let Some(usage) = message.get("usage") {
                        ...
                        anthropic_cache_read = usage.get("cache_read_input_tokens").and_then(|t| t.as_u64());
                        anthropic_cache_creation = usage.get("cache_creation_input_tokens").and_then(|t| t.as_u64());
                    }
                }
                continue;
            }
    ...
    TokenUsage { input_tokens: None, output_tokens: None, cache_hit_tokens: None, cache_miss_tokens: None }
}
...
fn estimate_tokens(body: &Value, _is_stream: bool) -> u64 {
    ...
    let input_chars = body
        ...
        .and_then(|m| {
            m.as_array().map(|arr| {
                arr.iter()
                    .filter_map(|msg| msg.get("content").and_then(|c| {
                        if let Some(s) = c.as_str() {
                            Some(s.len())
                        } else {
                            // Array content blocks
                            Some(c.as_array().map(|blocks| {
                                blocks.iter().filter_map(|b| b.get("text").and_then(|t| t.as_str()).map(|t| t.len())).sum::<usize>()
                            }).unwrap_or(0))
                        }
                    }))
                    .sum::<usize>()
...
pub(crate) async fn dispatch(
    ...
    let original_model = body
        .get("model")
        .and_then(|m| m.as_str())
        .unwrap_or(proxy_config.default_model)
        .to_string();
    ...
    // Extract session ID for affinity routing
    ...
    // Also coalesce duplicate in-flight requests: if a matching request is already
    // being processed, wait for it and reuse the cached result.
    if !is_stream {
        ...
        if let Some(cached) = state.request_cache.get(cache_key) {
            ...
            return json_response(reqwest::StatusCode::OK, cached);
        }
    ...
    let fallback_chain = router::fallback::resolve_fallback_chain(
        &original_model,
    ...
    let deadline = start
        + std::time::Duration::from_secs(
            state.request_timeout_secs.unwrap_or(120),
        );
    ...
    for current_model in &fallback_chain {
        ...
        while total_attempts < max_total_attempts {
            ...
            let channel = if let Some(aff_id) = affinity_channel {
            ...
            let channel = match channel {
                ...
                None => {
                    ...
                    match router::select_channel(channels.clone(), current_model, &state.routing_strategy, &state.active_requests).await {
                        Some(ch) => ch,
                        None => {
            ...
            let effective_auth = if channel.credential.cred_type == crate::channel::CredentialType::WebSession {
                AuthStyle::Cookie
            } else {
            ...
            if status.is_server_error() {
                ...
                state
                    ...
                    .log(make_log(
                        ...
                        channel.priority,
                        attempt,
                        Some(&format!("{}xx", status.as_u16() / 100)),
                        start.elapsed().as_millis() as u64,
                        false,
                        None,
                        None,
            ...
            if !status.is_success() {
                ...
                state
                    ...
                    .await;
                return json_response(status_code, body_text);
            }
            ...
            // Passive rate-limit extraction — update quota store from response headers
            {
                if let Some(qh) = crate::quota::collectors::response_header::QuotaHeaders::extract(
                    ...
                    state.quota_store.update_rate_limits(
                        channel.id,
            ...
            if is_stream {
                ...
                let estimated_cost = channel.calculate_cost(
                    Some(est_tokens / 2),
                    Some(est_tokens / 2),
                ).or_else(|| channel.cost_per_token.map(|rate| rate * est_tokens as f64 / 1000.0));
                ...
                let _ = state.channel_mgr.record_latency(channel.id, start.elapsed().as_millis() as u64).await;
                state.active_requests.decrement(channel.id);
                ...
                // Spawn background task to extract real token counts from stream
                {
                    ...
                    let bg_model = current_model.clone();
                    ...
                    let bg_cost_fn = channel.input_cost_per_mtok.is_some() || channel.output_cost_per_mtok.is_some();
                    ...
                    crate::spawn_bg(async move {
                        ...
                        if input_tokens.is_some() || output_tokens.is_some() {
                            ...
                            let real_cost = if bg_cost_fn {
                                ...
                                Some(((input_tokens.unwrap_or(0) + output_tokens.unwrap_or(0)) as f64 / 1000.0) * rate_per_1k)
                            } else {
                                None
                            };

                            if let (Some(it), Some(ot)) = (input_tokens, output_tokens) {
                                bg_logger.update_log_tokens(
                                    log_id,
                                    it,
                                    ot,
                            ...
                            bg_quota_store.accumulate_usage(
                                bg_channel_id,
                                input_tokens,
                                output_tokens,
                                token_usage.cache_hit_tokens,
                ...
                if let Some(secs) = state.stream_keepalive_secs {
                    if secs > 0 {
                        ...
                        let data_stream = body.into_data_stream().map(|r: Result<bytes::Bytes, axum::Error>| {
                ...
                state.active_requests.decrement(channel.id);
                return inject_passthrough_headers(json_response(StatusCode::OK, response_body), &upstream_headers);
            }
        }
    }

    // Log all-exhausted — also wake any coalesced waiters
    if !is_stream {
        let cache_key = crate::proxy::cache::RequestCache::cache_key(&original_model, body);
        state.in_flight.complete(cache_key);
    }
